...
void main()
{
    ...
    vec3 viewDirection = normalize(vec3(v_inv *
    ...
    for (int index = 0; index < numberOfLights; index++)
    {
            ...
            if (lights[index].spotCutoff <= 90.0) // spotlight?
            {
                float clampedCosine =
                    max(0.0, dot(-lightDirection,
                                 normalize(lights[index].spotDirection)));
                ...
                else
                {
                    attenuation = attenuation *
                        pow(clampedCosine, lights[index].spotExponent);
                }
            }
        }
        ...
        {
            specularReflection = attenuation * vec3(lights[index].specular) *
